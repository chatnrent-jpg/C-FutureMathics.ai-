#!/usr/bin/env python3
"""FutureMathics — Streamlit dashboard (polls data/system_state.json)."""

from __future__ import annotations

import json
import os
import sys
import time
from datetime import datetime, timezone
from pathlib import Path

import streamlit as st

ROOT = Path(__file__).resolve().parent.parent
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from engine.config import HANDSHAKE_EQUITY_BASE
from engine.ui_state_bridge import ensure_boot_system_state

STATE_PATH = ROOT / "data" / "system_state.json"
ensure_boot_system_state(STATE_PATH)


def load_state() -> dict:
    if not STATE_PATH.exists():
        return {}
    try:
        return json.loads(STATE_PATH.read_text(encoding="utf-8"))
    except Exception:
        return {}


def _state_age_seconds(data: dict) -> float | None:
    raw = data.get("updated_at") or (data.get("dashboard") or {}).get("as_of_utc")
    if not raw:
        return None
    try:
        ts = datetime.fromisoformat(str(raw).replace("Z", "+00:00").replace(" UTC", "+00:00"))
        if ts.tzinfo is None:
            ts = ts.replace(tzinfo=timezone.utc)
        return max(0.0, (datetime.now(timezone.utc) - ts).total_seconds())
    except Exception:
        return None


st.set_page_config(page_title="FutureMathics", layout="wide")
st.title("FutureMathics.ai")
st.caption("MES Micro E-mini S&P 500 — VWAP trend · Manus risk · paper mode")

if os.environ.get("FM_EXTERNAL_ORCHESTRATOR", "").strip().lower() in {"1", "true", "yes"}:
    st.caption("Cloud view — fed by background orchestrator (`system_state.json` poll)")

data = load_state()
age_s = _state_age_seconds(data)
dash = data.get("dashboard") or {}
session = data.get("session") or {}

if age_s is not None and age_s > 30:
    st.warning(
        f"Engine looks idle — last update {int(age_s)}s ago. "
        "Start the orchestrator: `launch_futuremathics.bat` or double-click **FutureMathics Local** (starts engine + dashboard)."
    )
nav = float(dash.get("account_nav") or data.get("account_nav") or HANDSHAKE_EQUITY_BASE)
pnl = float(dash.get("daily_pnl") or session.get("realized_pnl_today") or 0)
c1, c2, c3, c4 = st.columns(4)
c1.metric("Account NAV", f"${nav:,.2f}")
c2.metric("Daily P&L", f"${pnl:,.2f}", f"{dash.get('daily_pnl_pct', 0):+.2f}%")
c3.metric("Open risk", f"${session.get('open_risk_notional', 0):,.0f}", f"/ ${dash.get('max_concurrent_risk', 0):,.0f}")
c4.metric("Margin use", f"{dash.get('margin_utilization_pct', 0):.1f}%")
st.write(f"**Mode:** {dash.get('mode', '—')} · **Symbol:** {data.get('symbol', 'MES')} · **Updated:** {data.get('updated_at', '—')}")
st.write(f"**Risk:** {session.get('last_risk_verdict', '—')} — {session.get('last_risk_reason', '')}")
st.write(f"**Cycles:** {session.get('cycle_count', 0)} · **Trades today:** {session.get('trades_today', 0)}")
positions = data.get("open_positions") or []
if positions:
    st.subheader("Open positions")
    st.dataframe(positions, use_container_width=True)
else:
    st.info(
        "No open MES positions. Celine enters only when price is **≥4 ticks from VWAP** (BULL/BEAR). "
        "NEUTRAL regime = wait — normal with the internal price sim."
    )

time.sleep(2)
st.rerun()
